...
from snakemake_storage_plugin_gcs import StorageObject, StorageProvider, StorageProviderSettings
...
class TestStorageObject(unittest.TestCase):
    ...
    def test_list_candidate_matches(self, mock_client):
        ...
        storage_object = StorageObject(
            query='gs://mybucket/path/to/', keep_local=True,
            retrieve=True, provider=storage_provider
            )
        ...
        mock_client.bucket.assert_called_once_with(
            'mybucket',
            user_project=storage_object.provider.settings.project)
